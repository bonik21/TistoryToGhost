...
def convert_img_src(html_file, slug):
    with open(html_file, 'r+', encoding='utf-8') as file:
        html_content = file.read()
        html_content = html_content.replace('./img/', f'/content/images/old/{slug}/')
        file.seek(0)
...
def convert_file_src(html_file, slug):
    with open(html_file, 'r+', encoding='utf-8') as file:
        html_content = file.read()
        html_content = html_content.replace('./file/', f'/content/files/old/{slug}/')
        file.seek(0)
        file.write(html_content)
        file.truncate()        
